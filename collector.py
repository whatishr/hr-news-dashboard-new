...
아래 9개 sub_category 중 가장 적합한 하나를 선택하세요.
...
- 채용:
  신규채용, 경력채용, 채용전형 등
  새로운 인력을 채용하는 것이 핵심인 경우

- 인력변화:
  희망퇴직, 퇴직, 구조조정, 인력감축,
  인력재배치, 직무전환 등 기존 인력의 변화가 핵심인 경우
...
- 트렌드 · 타사 사례:
  특정 제도 하나의 변화보다
  여러 기업의 HR 사례나 업계 전체의 흐름·트렌드가
  핵심인 경우

[중요]

- 반드시 위 9개 중 하나만 선택합니다.
- 새로운 sub_category 이름을 만들지 않습니다.
- 기존 category는 절대 변경하지 않습니다.
- 단순히 특정 키워드가 등장했다는 이유만으로 분류하지 않습니다.
- 기사 제목과 최종 summary에서 드러나는 핵심 내용을 기준으로 판단합니다.
...
""".strip()
    ...
    if sub_category not in SUB_CATEGORY_ORDER:
        return "트렌드 · 타사 사례"
